...
import requests, os, json, time, uuid, re
...
# ---------- helpers ----------
def _headers():
    return {"Authorization": f"Bearer {HYPERBOLIC_API_KEY}", "Content-Type": JSON_CT}

def _post_chat(payload: dict) -> requests.Response:
    return requests.post(f"{HYPERBOLIC_API_URL}/chat/completions",
                         json=payload, headers=_headers(), timeout=DEFAULT_TIMEOUT)

def _stream_chat(payload: dict):
    """Relay SSE dari Hyperbolic apa adanya (passthrough)."""
    with requests.post(f"{HYPERBOLIC_API_URL}/chat/completions",
                       json=payload, headers=_headers(),
                       stream=True, timeout=DEFAULT_TIMEOUT) as r:
        r.raise_for_status()
        for line in r.iter_lines(decode_unicode=True):
            if line is None:  # keep-alive
                continue
            # pastikan diawali 'data: '
            yield (line if line.startswith("data:") else f"data: {line}") + "\n"
        # just in case upstream tak kirim [DONE]
        yield "data: [DONE]\n"
...
# ---------- anti-buffering untuk proxy/nginx ----------
@app.after_request
def _no_buffer(resp):
...
def list_models():
    ...
    models = [{
        "name": name, "model": name, "modified_at": "",
        "size": 0, "digest": "",
        "details": {"format":"openai-proxy","family":"meta-llama","parameter_size":"3B","precision":"fp16"}
    } for name in aliases]
...
def api_generate():
    try:
        ...
        return jsonify({
            ...
            "context": [], "total_duration":0,"load_duration":0,
