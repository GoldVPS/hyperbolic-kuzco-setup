...
MODEL_ALIASES      = [s.strip() for s in os.getenv("MODEL_ALIASES", "").split(",") if s.strip()]
...
def _ensure_messages(data: dict):
    """
    Terima payload gaya Ollama (prompt) atau OpenAI (messages) lalu
    normalisasi ke OpenAI messages.
    """
...
def _to_ollama_generate_response(content: str, model: str):
    # Response ala `ollama /api/generate`
    return {
        "model": model,
        "created_at": datetime.utcnow().isoformat() + "Z",
        "response": content,
...
def _post_chat_completions(payload: dict):
    # Pastikan stream False supaya mudah diparse
...
def root():
    return jsonify({
        "status": "ok",
        "service": "hyperbolic-ollama-compat",
...
def health():
    ...
    return jsonify({
        "status": "healthy" if ok else "missing_api_key",
        "model": HYPERBOLIC_MODEL,
        "aliases": MODEL_ALIASES,
    }), 200

# ---- OLLAMA COMPAT: /api/tags (untuk discovery) ----
...
def list_models_tags():
    ...
    for a in MODEL_ALIASES:
        ...
        models.append({
            ...
            "modified_at": "2024-01-01T00:00:00Z",
            "size": 3_000_000_000,
            "digest": f"hyperbolic-{ad}",
            "details": {"format": "gguf", "family": "llama", "parameter_size": "3B"},
        })
    return jsonify({"models": models})

# ---- OPENAI COMPAT: /v1/models (dibutuhkan inference-cli saat reconcile) ----
...
def v1_models():
    ...
    return jsonify({"object": "list", "data": items}), 200

@app.route("/v1/models/<model_id>", methods=["GET"])
def v1_model_detail(model_id):
    if model_id == HYPERBOLIC_MODEL or model_id in MODEL_ALIASES:
        return jsonify({
            "id": model_id,
            "object": "model",
            "created": 0,
...
# ---- OPENAI STYLE: pass-through (untuk tools yang nembak langsung) ----
@app.route("/v1/chat/completions", methods=["POST"])
def v1_chat_completions_passthrough():
    try:
        data = request.get_json(force=True) or {}
        data["model"] = HYPERBOLIC_MODEL  # paksa konsisten
        r = SESSION.post(
            f"{HYPERBOLIC_API_URL}/chat/completions",
            headers=_headers(),
            json=data,
...
# (opsional) kompat tanpa prefix saat uji manual
...
if __name__ == "__main__":
    # Gunicorn disarankan di Docker, ini supaya bisa run lokal
